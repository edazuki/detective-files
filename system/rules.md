...
- 場所・人物との会話などのアクションで `clues.md` の取得条件を満たすと、AIはその時点のシナリオ文脈（日時・立会人・発見状況）を反映して証拠ファイルを生成し `evidence/` に保存する
- **証拠ファイル・聴取メモを `evidence/` に追加・生成したら、その場で `evidence-data.js` を更新する**
...
- フラグの現在値は `saves/<セーブ名>/state.yml` の `flags` セクションで管理する
...
characters:
  [character_id]:
    ...
    acquired_knowledge:
      - "[プレイヤーとの会話・開示でこのキャラクターが得た情報]"
    last_interaction: "[最後の会話の要約]"
```

> **`acquired_knowledge` の更新ルール**: 会話後に `state.yml` を更新する際、そのキャラクターが今回の会話で新たに知った情報を追記する。`characters.md` の `acquired` フィールドは廃止し、このフィールドで一元管理する。

### Emergent NPC（`characters.md` 未定義の人物）

プレイヤーが `characters.md` に定義されていない人物への接触を試みた場合：

1. `evidence/` 内の証拠ファイルにその人物への言及があるか確認する
2. 言及がある場合、そこから推論できる範囲で人物像を即興で構築する
3. `state.yml` の `characters` セクションに初期値を追加して、以降は通常キャラクターと同様に管理する
4. 言及がない場合は「その人物については情報がない」として対応する
...
```
saves/<セーブ名>/evidence/    ← プレイヤーの「捜査机」。生成済みの証拠ファイルと聴取メモが並ぶ
saves/<セーブ名>/evidence-data.js  ← viewer.html 用データ。証拠追加のたびに更新する
```
...
| `dest` | ✅ | 生成ファイルの保存先。ファイル名stemを `id` と一致させる（例: `evidence/[id].txt`） |
| `condition` | ✅ | 取得可能になる条件（フラグ・場所・アクション・好感度） |
| `proves` | ✅ | この証拠が証明する内容。AIがファイル生成時に含めるべき核心情報 |
| `acquisition_context` | - | 入手の経緯・AIへのメモ（文書の性質・入手タイミングの指針など） |
| `description` | - | AIが画像等を読めない場合の代替説明文 |
| `display_name` | - | プレイヤーへの表示名（日本語等、見せてよい名前）。省略時はidを使用 |
| `initial` | - | `true` の場合、セッション開始時に条件なしで自動生成される。省略時は `false` |
...
> **「話してくれなかったこと」の記載ルール**: 探偵が実際に質問し、NPCがはぐらかした・拒否した・明らかに回避した事柄のみを記載する。聞いていない情報をAIが推測で「話さなかった」として列挙しない。
...
フラグ・好感度・`state.yml` の内容はプレイヤーに見せない内部情報。選択肢や案内文に「〇〇フラグあり」「条件クリア済み」などの記述を入れない。場所や行動が「できる/できない」の理由は、世界内の理由として自然に描写する。

### プレイヤー発言の解釈

プレイヤーの発言は、明示的なメタ宣言がない限りゲーム内発言として扱う。「ゲームの外から言うと」「メタ発言だけど」「OOC:」などの宣言がある場合のみメタ発言と判断する。

### パートナーキャラクターの管理

パートナーはシナリオの `characters.md` にNPCとして定義し、`state.yml` の `characters` セクションにも他NPCと同様に追加する。AIは `truth.md` の全情報を持つが、パートナーはシナリオ内で知り得た情報のみを持つキャラクターとして動かす。
...
- id: gray
  knowledge:
    ...
    concealed:
      ...
      - "帳簿を横領していたこと"
      - "遺書が偽物であること"
# acquired_knowledge はプレイ中に state.yml で管理する
```
...
各キャラクターに `status` フィールドを定義することで、会話可否をAIが制御する。

| 値 | 意味 |
|---|---|
| `active` | 通常のNPC。会話・好感度変動が有効（省略時のデフォルト） |
| `deceased` | セッション開始時点で故人。会話不可。`knowledge` は証拠ファイル解釈に使用 |
| `deceased_when: [flag_name]` | 指定フラグが立った瞬間から故人扱いになる。それ以前は `active` |

```yaml
# characters.md での記述例
- id: edward
  status: deceased          # 最初から故人

- id: witness
  status:
    deceased_when: witness_killed   # フラグが立つまでは会話可能
```

> **故人キャラクターを定義する意義**: 会話はできないが、AIが「この人物はどんな人だったか」を把握することで、日記・手紙・証言の解釈に一貫性が生まれる。`knowledge` の定義は必須ではないが、あると精度が上がる。
