...
証拠の入手手段を有向非巡回グラフ（DAG）として整理することで、重要な証拠へのアクセスの難しさや、伏線の配置など、シナリオを調整しやすくなります。循環依存（証拠AにはBが必要でBにはAが必要）はデッドロックを生むため避けること。
証拠ファイルは必ずしも事件の証拠となるものである必要はありません。シナリオに関わらない設定をフレーバーとして証拠ファイルに仕込むこともできます。
